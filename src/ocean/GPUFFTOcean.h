...
public:
    ...
    // read back to the CPU once per render frame; sampleDisplacement returns the
    // (dx, height, dz) wave displacement (metres) at a world XZ, matching the GPU.
    ...
    // world (X,Z) came from a different grid point. This inverts that mapping
